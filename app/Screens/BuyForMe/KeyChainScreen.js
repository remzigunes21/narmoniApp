...
class KeychainScreen extends BaseScreen {
  ...
  async save() {
    try {
      let start = new Date();
      const {username, password, vendor} = this.state;
      //   await this.props.store.KeyStore.setCredentials(username, password)
      ...
      AsyncStorage.getItem('vendor').then(async vendor => {
        await this.props.store.buyForMeStore.login(vendor, username, password);
      });
  ...
  async load() {
    try {
      await this.props.store.KeyStore.getCredentials();
    } catch (err) {
